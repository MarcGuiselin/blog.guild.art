...
export default function EmbedTweet({
  ...
  return (
    <div className="not-prose tweet rounded-lg border border-slate-300 dark:border-onyx-700 px-6 py-4 my-4 w-full bg-white dark:bg-onyx-800 dark:bg-opacity-0 dark:hover:bg-opacity-100 transition">
      <div className="flex items-center">
        ...
        <a
          ...
          className="author flex flex-col ml-4 group"
        >
          <span
            className="flex items-center font-bold text-slate-900 dark:text-gray-100 leading-5 group-hover:!underline"
            title={author.name}
          >
          ...
          <span
            className="text-slate-500 dark:text-white dark:text-opacity-50 tracking-wide"
            title={`@${author.id}`}
          >
            @{author.id}
          </span>
      ...
      <Twemoji
        tag="div"
        className="mt-4 mb-1 leading-normal whitespace-pre-wrap text-xl !text-slate-700 dark:!text-slate-300"
        text={formattedText}
      />
      {media && media.length ? (
        <div
      ...
      <a className="!text-slate-500" href={tweetUrl} target="_blank" rel="noopener noreferrer">
        <time title={`Time Posted: ${date.toUTCString()}`} dateTime={date.toISOString()}>
          {formatDate(date)}
        </time>
      </a>
      <div className="flex !text-slate-700 dark:!text-slate-300 mt-2 pt-3 border-t border-slate-300 dark:border-onyx-700">
        <a
          className="flex items-center mr-4 !text-slate-500 hover:!text-blue-600 transition hover:!underline"
          href={replyUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <a
          className="flex items-center mr-4 !text-slate-500 hover:!text-green-600 transition hover:!underline"
          href={retweetUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <a
          className="flex items-center !text-slate-500 hover:!text-red-600 transition hover:!underline"
          href={likeUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
